...
std::string NCursesWrapper::GetHelloWorld() { return "**** Hello World ****"; }
std::string NCursesWrapper::GetLongText() {
...
void NCursesWrapper::putpixel(int i, int j, const std::string &str,
                        bool do_refresh) const {
...
void NCursesWrapper::PrintString(int i, int j, const std::string &str,
                           bool do_refresh) const {
  mvwprintw(stdscr, i, j, "%s", str.c_str());
  if (do_refresh) {
    refresh();
...
void NCursesWrapper::ScrollText(std::string_view text, int intervalMilliSeconds,
                          int y) {
...
void NCursesWrapper::drawcircle(int i0, int j0, int radius) {
  int i = radius;
  int j = 0;
  int err = 0;
...
bool NCursesWrapper::DrawLine(const int i0, const int j0, const int i1, const int j1,
...
bool NCursesWrapper::DrawLine(const Point &start, const Point &end,
                        const std::string &str) const {
  return DrawLine(start.GetI(), start.GetJ(), end.GetI(), end.GetJ(), str);
}

bool NCursesWrapper::DrawSevenSegment(const int i0, const int j0, const int width,
...
void NCursesWrapper::PrintStringOnSevenSegment(int i, int j, int width, int height,
